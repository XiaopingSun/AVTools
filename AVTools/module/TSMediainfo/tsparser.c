...
/**
 * Parse TS Packet
 * @param parser               TSParser Instance
 * @param packet_buffer     TS Packet Data Pointer
 * @param packet_size        TS Packet Buffer Size
 */
void parseTSPacket(TSParser *parser, uint8_t *packet_buffer, size_t packet_size) {
    
    ABitReader bitReader;
    initABitReader(&bitReader, packet_buffer, packet_size);
    ...
    // 解析TS Packet Header
    {
        color_print(COLOR_FT_WHITE, COLOR_BG_NONE, "==================== Start Parsing TS Packet Header ====================\n");
        sync_byte = getBits(&bitReader, 8);
        printf("Sync Byte: 0x%02x\n", sync_byte);

        transport_error_indicator = getBits(&bitReader, 1);
        printf("Transport Error Indicator: %u\n", transport_error_indicator);
        ...
        color_print(COLOR_FT_WHITE, COLOR_BG_NONE, "===================== End Parsing TS Packet Header =====================\n\n");
    }
    
    // 解析TS Packet Adaptation Field
	if(adaptation_field_control == 2 || adaptation_field_control == 3) {
		parseAdaptationField(parser, &bitReader);
	}

    // 根据PID解析TS Packet Payload
...
/**
 * Parse TS Packet Adaptation Field
 * @param parser           TSParser Instance
 * @param bitReader       TSParser Instance
 */
...
/**
 * Parse TS Packet Program Id
 * @param parser                                      TSParser Instance
 * @param bitReader                                  TSParser Instance
 * @param pid                                           Program Id
 * @param payload_unit_start_indicator        Payload Unit Start Indicator In TS Packet Header
 */
void parseProgramId(TSParser *parser, ABitReader *bitReader, uint32_t pid, uint32_t payload_unit_start_indicator) {
	...
	if (pid == 0) {
        // 如果承载PSI的TS Packet Header的payload_unit_start_indicator为1  则载荷首字节承载pointer_field  PSI数据在 pointer_field 指示大小之后 因此需要跳过
    ...
    for (listItem = parser->mPrograms.mHead; listItem != NULL; listItem = listItem->mNext) {
        ...
        if(pid == pProgram->mProgramMapPID) {
            ...
            // 解析PMT
            parseProgramMapTable(parser, pProgram, bitReader);
            handled = 1;
            break;
        } else {
            // 解析PES
...
void parseProgramAssociationTable(TSParser *parser, ABitReader *bitReader) {
    ...
    color_print(COLOR_FT_YELLOW, COLOR_BG_NONE, "========================== Start Parsing PAT ===========================\n");
    ...
    color_print(COLOR_FT_YELLOW, COLOR_BG_NONE, "=========================== End Parsing PAT ============================\n\n");
}

/**
 * Parse Stream
 * @param stream                                    TSStream Instance
 * @param payload_unit_start_indicator       Payload Unit Start Indicator In TS Packet Header
...
/**
 * Parse PES                                            https://blog.csdn.net/u013354805/article/details/51591229
 * @param stream                                    TSStream Instance
 * @param bitReader                                ABitReader Instance
 */
void parsePES(TSStream *stream, ABitReader *bitReader) {
    
    color_print(COLOR_FT_DARK_GREEN, COLOR_BG_NONE, "======================= Start Parsing Former PES =======================\n");
    
    // packet_start_code_prefix  0x000001
    getBits(bitReader, 24);
    ...
    color_print(COLOR_FT_DARK_GREEN, COLOR_BG_NONE, "======================== End Parsing Former PES ========================\n\n");
}
...
/**
 * Function Called When We Have PES Payload Content
 * @param stream                                      TSStream Instance
 * @param PTS_DTS_flag                            PTS_DTS_flag In PES Header
 * @param PTS                                         PTS In PES Header
 * @param DTS                                         DTS In PES Header
 * @param data                                        PES Payload Pointer In 188Byte Buffer
 * @param size                                         Size Of PES Payload
 */
...
/**
 * Add Program Into Parser Program List
 * @param parser                                      TSParser Instance
 * @param programMapPID                          Program Map Id
 */
...
/**
 * Add Stream Into Program
 * @param program                            TSProgram Instance
 * @param elementaryPID                    Elementary Id
...
/**
 * Get TSStream With PID In TSProgram
 * @param program                 TSProgram Instance
 * @param pid                        Program Id
 */
 ...
 * @param bitReader                         ABitReader Instance
 */
void parseProgramMapTable(TSParser *parser, TSProgram *program, ABitReader *bitReader) {
    
    color_print(COLOR_FT_PURPLE, COLOR_BG_NONE, "========================== Start Parsing PMT ===========================\n");
    ...
    color_print(COLOR_FT_PURPLE, COLOR_BG_NONE, "=========================== End Parsing PMT ============================\n\n");
}
